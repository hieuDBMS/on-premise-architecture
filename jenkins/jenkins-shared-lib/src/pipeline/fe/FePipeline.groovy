...
class FePipeline extends Jenkins {
    def cdPipeline
    def ciPipeline
    def qualityGate
    def buildStatus
    ...
    def run(String gitUrl, String cdRepoUrl, String deployEnv, String appName, String serviceName, String timeZoneId) {
        execute {
            try {
                ...
                this.resultAnalysis = this.qualityGate.sonarQubeAnalysisFE(appName, serviceName)
                ciPipeline.build(deployEnv, appName, serviceName)
                cdPipeline.commonStages(deployEnv, appName, serviceName, this.imageTag, cdRepoUrl, timeZoneId)
