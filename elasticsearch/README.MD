...
2. **Label Your Worker Node to have node-role=data-es**. For example:
```
kubectl label node k8s-worker-1 node-role=data-es --overwrite
```
...
- The Data Node connect to the Master Node using:
    ...
    3. nodeSelector: Select the worker node with node-role=data-es
